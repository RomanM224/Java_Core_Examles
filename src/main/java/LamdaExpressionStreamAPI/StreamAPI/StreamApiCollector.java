...
public class StreamApiCollector {
    ...
    // множества
    // toConcurrentMap, toMap - позволяют преобразовать стрим в map, используя
    // указанные функции
    ...
    // partitioningBy - разделяет коллекцию на две части по соотвествию условию и
    // возвращает их как Map<Boolean, List>
    // groupingBy - разделить коллекцию по условию и вернуть Map<N, List<T>>, где T
    // - тип последнего стрима, N - значение разделителя
    ...
    private static void testCollect() {
        ...
        System.out.println("statistics = " + statistics); // напечатает statistics = IntSummaryStatistics{count=4,
                                                          // sum=22, min=4, average=5.500000, max=7}
        ...
        // Преобразовать в map, сгрупировав по первому символу строки и в качестве
        // значения взять второй символ объединим через :
        ...
        // Напишем собственный Collector, который будет выполнять объединение строк с
        // помощью StringBuilder
        Collector<String, StringBuilder, String> stringBuilderCollector = Collector.of(StringBuilder::new, // метод
                                                                                                           // инициализации
                                                                                                           // аккумулятора
                (b, s) -> b.append(s).append(" , "), // метод обработки каждого элемента
                (b1, b2) -> b1.append(b2).append(" , "), // метод соединения двух аккумуляторов при параллельном
                                                         // выполнении
